import sys
from PySide6.QtWidgets import QApplication, QDialog, QMainWindow, QVBoxLayout, QTextEdit, QPushButton
...
class VOIE():
    def __init__(self):
        self.voies = [VOIE_NONE for _ in range(NOMBRE_VOIES)]
voies = VOIE()
...
class MainWindow(QMainWindow):
    ...
    def RxManage(self):
        ...
        match com.rxMsg[self.FIFO_lecture].id:
            ...
            case 0xB2:#ID_TEST_EN_COURS
                ...
                self.ui.textEdit_panel.append(f"ID_TEST_EN_COURS : bilan : " + str(voies.voies))
            case 0xB3:#ID_TEST_TERMINEE
                voies.voies = com.rxMsg[self.FIFO_lecture].data
                self.ui.textEdit_panel.append(f"ID_TEST_TERMINEE : bilan : " + str(voies.voies))
    ...
    def sendByte(self, id, byte):
        sample_message = Message(id, length=1, data=[byte])
...
class Dialog(QDialog):
    ...
    def start_serial(self):
        ...
        if selected_port:
            try:
                com.serial_thread = SerialThread(selected_port, 921600)
